...
const EditUserRolePage = () => {
  const [userInfo, setUserInfo] = useState<UsersData>({ users: [], page: 1, total: 0, limit: 10, totalPages: 0 }); // All users
  ...
  const handleEditRoleDialog = (role?: string, success?: boolean, userId?: string) => {
    if (success && role && userId) {
      ...
      setUserInfo((prevUserInfo) => ({
        ...prevUserInfo,
        users: prevUserInfo.users.map((user) => (user.id === userId ? { ...user, role } : user)),
      }));
    }
  };
